import React, {useState} from 'react';
import {Collapse} from "antd";
import h_1 from "@/assets/production/production_s3/DEW06342.jpg";
import h_2 from "@/assets/production/production_s3/DEW06362.jpg";
import h_3 from "@/assets/production/production_s3/DEW06333.jpg";
import h_4 from "@/assets/production/production_s3/DEW06330.jpg";
import {useTranslation} from "react-i18next";


const ProductionS3 = () => {
    const [activeKey, setActiveKey] = useState('1');
    const {t} = useTranslation();
    const items = [
        {
            key: '1',
            label: t('production.production_s3.i1.t'),
            children: <p>{t('production.production_s3.i1.d')}</p>,
        },
        {
            key: '2',
            label: t('production.production_s3.i2.t'),
            children: <p>{t('production.production_s3.i2.d')}</p>,
        },
        {
            key: '3',
            label: t('production.production_s3.i3.t'),
            children: <p>{t('production.production_s3.i3.d')}</p>,
        },
        {
            key: '4',
            label: t('production.production_s3.i4.t'),
            children: <p>{t('production.production_s3.i4.d')}</p>,
        },
    ];

    const imageMap = {
        '1': h_1,
        '2': h_2,
        '3': h_3,
        '4': h_4,
    };
    const onChange = (key) => {
        if (!key || (Array.isArray(key) && key.length === 0)) return;
        setActiveKey(Array.isArray(key) ? key[0] : key);
    };
    return (
        <section className={"current-container production_s1"}>
            <div className="production_s1_title col-lg-6">
                <h2 className={"production_title"}>{t("production.production_s3.title")}</h2>
            </div>
            <div className="row">
                <div className="col-lg-5 mt-5 ">
                    <Collapse
                        accordion
                        activeKey={activeKey}
                        onChange={onChange}
                        items={items}
                        expandIconPosition={"end"}
                    />
                </div>
                <div className="col-lg-7 ">
                    <div className="production_s1_img">
                        <img src={imageMap[activeKey]} alt=""/>
                    </div>
                </div>
            </div>
        </section>
    );
};

export default ProductionS3;